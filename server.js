...
if (!groq) {
    console.warn("⚠️ Warning: GROQ_API_KEY is not set in environment variables. AI chat endpoint will be disabled.");
}

app.post("/api/chat", async (req, res) => {
    try {
        if (!groq) {
            return res.status(503).json({ reply: "AI Assistant is currently offline. Please set GROQ_API_KEY in Render environment variables." });
        }
